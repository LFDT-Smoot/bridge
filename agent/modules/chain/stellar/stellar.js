 ...
 */
const TimeoutPromise = require('@framework/utils/timeoutPromise')
const abstract_base_chain = require("@framework/chain/chain_abstract");
const {hexTrip0x, hexToAscii} = require("@framework/comm/lib");

const sorobanClient = require('./utils/client_soroban')
    ...
    let scanOneBlock = async function (blockNum) {
      try {
        ...
        for (const tx of txs) {
          ...
          if(!txMeta) {
            // to get tx meta data
            const txInfo = await that.client.getTransactionDetails(tx.hash);
            txMeta = txInfo.resultMetaXdr;
          }

          if(undefined === txMeta){
            that.log.warn("Stellar failed to get 'resultMetaXdr' info for txHash: ", txHash);
            continue;
          }
        ...
        // console.log("...matched Events: ", foundEvents);
        return foundEvents;
