...
let keyStore = require("@framework/utils/keyStore.js");

const { FeeMarketEIP1559Transaction } = require( '@ethereumjs/tx' );

let Transaction;
// compatible with old and new version 'ethereumjs-tx'
if (typeof ethTx === 'object' && ethTx.Transaction) {
	Transaction = ethTx.Transaction; // Use the Transaction from the object
} else if (typeof ethTx === 'function') {
	Transaction = ethTx; // Use the function directly
} else {
	throw new Error('Transaction function is not available.');
}
...
module.exports = class EthRawTransModel {
	...
	serialize(signature) {
		let tx;
		tx = new Transaction(this.txParams);
