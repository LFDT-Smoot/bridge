...
const {
    ...
    XdrLargeInt,
    Address,
    nativeToScVal, xdr, scValToNative, ScInt
} = require("@stellar/stellar-sdk");
const {sha3, keccak256} = require('ethereumjs-util')

const data = require('./data_cancel_order_template');
let cancel = true;
let cancle = true;
...
const run = async ()=> {
    ...
    console.log('encodeInfo: ', encodeInfo.toString('hex'));
    const sha256hash = keccak256(encodeInfo)


    const user1_secp256k1_privKey = Buffer.from(config.secp256k1.user5.privKey,'hex');
