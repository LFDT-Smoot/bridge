...
pub fn decode_and_verify(env: &Env,  encoded_info: Bytes,
    ...
    env.events().publish(("decode_and_verify--0, ",),data_hash_v.clone());
    ...
    for i in 0..l {
        ...
        env.events().publish(("decode_and_verify--1",),pubkey.clone());
        let j = find_pubkey(&env, pubkey.clone());
        env.events().publish(("decode_and_verify--2",),j.clone());
        if j == 0 {
        ...
        env.events().publish(("decode_and_verify--3",),"success10");
    }
